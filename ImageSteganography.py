...
class Stegno:
    ...
    art2 = '''Digital Image Processing

    Submitted By:

    Kunal Tilaganji  PA51
    Sourabh Deshpande PA41
    Ankita Kumari PA58'''
    output_image_size = 0
    ...
    def main(self,root):
        ...
        title.config(font=('Times New Roman',33, "italic"))
        title.grid(pady=10)
    #Tried Different Fonts for The Headings
        b_encode = Button(f,text="Encode",command= lambda :self.frame1_encode(f), padx=14)
        b_encode.config(font=('Times New Roman',14))
        b_decode = Button(f, text="Decode",padx=14,command=lambda :self.frame1_decode(f))
        b_decode.config(font=('Times New Roman',14))
    ...
    def frame1_decode(self,f):
        f.destroy()
        d_f2 = Frame(root)
        label_art = Label(d_f2, text='Lets Decode!')
